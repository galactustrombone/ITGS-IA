	...
	<body>
		<div id="content">
			...
			if($_SERVER["REQUEST_METHOD"] == "GET") {
				...
				$search = "SELECT Piece.Name AS PieceName, Part.ID, Instrument, Quantity, LastBorrowedBy, DateBorrowed, Status FROM Part INNER JOIN Piece ON Part.PieceID = Piece.ID WHERE Status != 0 AND (INSTR(Piece.Name, '" . $query . "') > 0 OR INSTR(Instrument, '" . $query ."') > 0 OR INSTR(Quantity, '" . $query . "') > 0 OR INSTR(LastBorrowedBy, '" . $query . "') > 0) ORDER BY Piece.Name";
